...
허깅페이스 허브에 업데이트하면 다음과 같이 허깅페이스 허브에 자신만의 모델이 올라갑니다. 이 모델은 다른 모든 허깅페이스 모델과 똑같이 사용할 수 있습니다!

<img width="815" alt="image" src="https://github.com/jiogenes/utterances/assets/43975730/348f8a69-cd3a-4e7d-ba15-0781f82c25df">

마지막으로 모델의 성능을 평가해 봅니다.

```python
prompt = '<usr> 트랜스포머에 대해 설명해줘\n<bot> '
...
런팟을 이용해서 LLM을 학습하는 실습을 진행해 보았습니다. 저는 약 5시간 정도가 걸렸고 시간당 0.29달러로 약 2달러가 안되는 돈으로 Llama2 모델을 파인튜닝 할 수 있었습니다.

데이터 전처리와 다양한 학습 방법들을 적용한다면 이것보다 더 좋은 결과가 나올것입니다. 좋은 LLM은 단지 데이터만 많이 넣고 돌리는 수준에서 만들어 지는게 아니라 수많은 삽질(데이터 전처리)에 삽질(아키텍쳐 수정)을 거듭해서 나온 결과물일 것입니다.
